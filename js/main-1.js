export var availableFrames;

export function convertCharsToSize(
	charCount,
	penaltyCeiling = 0,
...
export function getBaseSize(key, text) {
	switch(key) {
		case "title":
...
export function notify(message = '', seconds) {
	...
	if (seconds) {
		setTimeout(() => { close.click(); }, seconds * 1000)
	}
}
...
export function dropEnter(e) {
	e.preventDefault();
	e.stopPropagation();
	e.target.closest('.drop-area').classList.add('hover');
}
export function dropLeave(e) {
	e.preventDefault();
	e.stopPropagation();
	e.target.closest('.drop-area').classList.remove('hover');
}
export function dropOver(e) {
	e.preventDefault();
	e.stopPropagation();
	e.target.closest('.drop-area').classList.add('hover');
}
export function dropDrop(e) {
	e.preventDefault();
	e.stopPropagation();
	e.target.closest('.drop-area').classList.remove('hover');
	destination = window[e.target.closest('.drop-area')?.querySelector("input").getAttribute('data-dropexport function')];
...
export async function uploadFiles(filesRaw, destination, otherParams = '') {
	var files = ([...filesRaw]);
	if (files.length > 9) {
...
export function bindInputs(query1, query2, checkbox = false) {
	var e1 = document.querySelector(query1);
	var e2 = document.querySelector(query2);

	if(!e1 || !e2) {
		console.warn("missing element queries")
		return;
	}
